...
class TestRegex {
	...
	void testNormal() {
		...
		expected.add(new Product("powder", 10.0, "", "a fun time", "disney", 0, null));
		expected.add(new Product("oil", 100.0, "", "Don't tell america", "Earth", 0,  null));
		expected.add(new Product("doot", 100.0, "", "dootdootdoot", "nootnoot", 0,  null));
		Mockito.when(pRepo.findProductByNameRegex(regex)).thenReturn(expected);
